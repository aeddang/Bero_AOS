...
enum class PageSizeType{
    Small, Medium;
    companion object {
        fun getType(resIdx:Int):PageSizeType{
            return when(resIdx){
                1 -> PageSizeType.Medium
                else -> PageSizeType.Small
            }
        }
    }
}
...
abstract class PageUI : FrameLayout, Page {
    ...
    protected open fun initialize(context: Context, attrs: AttributeSet?) {
        ...
        attrs?.let {
            ...
            if (style.hasValue(R.styleable.PageUIStyle_defaultStrokeColor)) {
                this.defaultStrokeColor = style.getColor(R.styleable.PageUIStyle_defaultStrokeColor, 0)
            }
            if (style.hasValue(R.styleable.PageUIStyle_defaultStrokeWidth)) {
                this.defaultStroke = style.getDimension(R.styleable.PageUIStyle_defaultStrokeWidth, 0f)
            }
            ...
            if (style.hasValue(R.styleable.PageUIStyle_defaultContentColor)) {
                this.defaultContentColor = style.getColor(R.styleable.PageUIStyle_defaultContentColor, 0)
            }
            if (style.hasValue(R.styleable.PageUIStyle_activeContentColor)) {
                this.activeContentColor = style.getColor(R.styleable.PageUIStyle_activeContentColor, 0)
            }
    ...
    protected var defaultContentColor:Int? = null
    protected var defaultTextSize:Float?  = null
    protected var activeContentColor:Int? = null
